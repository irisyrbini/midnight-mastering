...
- **Smoking (layered over locomotion)** — smoking never locks the producer in place. `SmokingEffect` is just the arm and the smoke, mounted *inside* the body rig (the bobbing/turning group of `WalkingFigure`, or the seated torso), so it **blends over whatever the lower body is doing**: the legs keep striding while the cigarette turns and bobs with the body, and the player can walk, run, steer and interact throughout. Lighting a cigarette does **not** raise the inspect card (`activeVideoId` stays unset for `cigarettes`). One drag plays through once: a plain **white** cigarette with a buff filter and a **red coal** at the tip (which brightens, with a matching point light, while drawing) — raise to the mouth (1.1s) → inhale (0.9s) → hold the breath (0.5s) → lower while exhaling (1.2s) → rest (1.3s). That is ~5s, exactly how long `smokingMinutes` lasts; then the cigarette **scales out and locomotion returns smoothly** (a `fade` weight eases the arm in and out). The phase **clamps rather than wraps**, so it never restarts mid-way. Smoke puffs are **world-anchored** (the pool's matrix is pinned to the identity), so they hang in the air where they were emitted and the producer walks out of them instead of dragging the cloud around on every turn: a thin trickle keeps rising off the lit tip, and a slower, wider cloud leaves the mouth after each inhale.
